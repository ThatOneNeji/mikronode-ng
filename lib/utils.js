 ...
 */
module.exports.decodeLength = function decodeLength(data) { // Ported from the PHP API on the
    // Wiki. Thanks
    let idx = 0;
    let b = data[idx++];
    let len;
 ...
 * @param {array} data - The data to dump
 */
module.exports.hexDump = function hexDump(data, debugSocketData) {
    let hex = [];
    let cref = [];
    let i = 0;
    for (let j = 0; j < data.length; j++) {
        ...
        if ((data[j] < 20) || (data[j] > 126)) {
            ...
            cref[i] = String.fromCodePoint(data[j]);
        }
 ...
 *           allowed.
 * @param {boolean} [cloneAndFreeze=false] - If true, the getter will return a deep
 *           cloned, frozen object
 */
module.exports.createProperty = function createProperty(object, name, initialValue, allowSet, privateStore,
    cloneAndFreeze) {
    let props = {
        enumerable: true
    };
    if (privateStore) {
        ...
        if (cloneAndFreeze) {
            props.get = function () {
                return Object.freeze(JSON.parse(JSON.stringify(privateStore(object)[name])));
            };
        } else {
            props.get = function () {
...
/**
 * Creates a listener whose 'this' is specified
 * @function
 * @private
 * @param {function} func - The function to call
 * @param {object} _this - The context in which to call it
 */
module.exports.makeListener = function makeListener(func, _this) {
    return function madeListener() {
