...
module.exports = (function () {
    ...
    Connection.prototype.loginStateMachine = function loginStateMachine() {
        ...
        // lc.write(['/login', '=name=', this.user, '=password=', this.password]);
        lc.write('/login');
        ...
        lc.on('done', function loginDone(data) {
            ...
            switch (_(_this).status) {
                ...
                case 'Waiting for challenge':
                    ...
                    const challenge = new Buffer.alloc(_(_this).password.length + 17);
                    challenge.write(emptyString + _(_this).password, 0, 'utf8');
                    challenge.write(parsed[0].ret, _(_this).password.length + 1, 'hex');

                    const resp = '00' + crypto.createHash('MD5').update(challenge).digest('hex');

                    _(_this).status = 'Sending Credentials';
                    debugLogin('%s %s %s %s', _(_this).status, _(_this).user, _(_this).password, resp);
                    this.write('/login', {
                        "=name": _(_this).user,
