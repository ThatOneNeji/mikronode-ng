...
module.exports = (function () {
    ...
    const debugChannel = dbg('mikronode-ng:channel');
    const debugChannelData = dbg('mikronode-ng:channel:data');

    const _ = require('private-parts').createKey();

    /**
     * writeCallback
     * @callback mikronode-ng.Channel.writeCallback
     * @param {Channel}
     */

    /**
     * Emitted when a command has finished successfully.
     * @event mikronode-ng.Channel#event:done
     ...
     * {@link mikronode-ng.Channel#close} or when the channel is closed automatically via
     * {@link mikronode-ng.Channel#closeOnDone}
     * @event mikronode-ng.Channel#event:close
     * @property {Channel} channel - The channel originating the event
     */
    /**
     * Emitted when a command has failed. Subsequent commands may succeed.
     * @event mikronode-ng.Channel#event:trap
     * @property {mikronode-ng.Trap} trap - The trap object
     */

    /**
     * Channel (should not be instantiated directly)
     * @exports mikronode-ng.Channel
     * @implements {EventEmitter}
     * @class
     * @param {number} id
     * @param {mikronode-ng.Connection} conn
     * @fires mikronode-ng.Channel#event:done
     * @fires mikronode-ng.Channel#event:trap
     * @fires mikronode-ng.Channel#event:error
     * @fires mikronode-ng.Channel#event:timeout
     * @fires {mikronode-ng.Channel#event:close}
     */
    function Channel(id, conn) {
         ...
         * @readonly
         * @instance
         * @member {mikronode-ng.Connection} connection
         * @memberof mikronode-ng.Channel
         */
         ...
         * @private
         * @instance
         * @member {mikronode-ng.Channel.writeCallback} writeCallback
         * @memberof mikronode-ng.Channel
         */
     ...
     * </pre>
     *
     * @param {mikronode-ng.Channel.writeCallback} [writeCallback] - This will be called
     *           just before write actually writes the data to the connection.
     */
    Channel.prototype.write = function write(d, parameters, writeCallback) {
        ...
        delete _(this).connection.getTraps()[_(this).id];
        if (d) {
            if (typeof (d) === 'string') {
                d = d.split("\n");
            }
    ...
    Channel.prototype._done = function _done(data, trap) {
        ...
        if (_(this).commands.length) {
            let c = _(this).commands.shift();
            let cl = _(this).closing;
     ...
     * {@link mikronode-ng.Connection#closeOnDone} was set and this was the last channel to
     * close.
     * @public
    ...
    /**
     * Calls {@link mikronode-ng.Channel#close}(false)
     * @public
     */
